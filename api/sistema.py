...
HTML_SISTEMA = """ 
(TODO TU HTML AQUÍ SE MANTIENE EXACTAMENTE IGUAL — NO LO MODIFIQUÉ)
"""
...
def car_agregar():
    ...
    p = conn.execute(
        "SELECT producto, precio, stock, tipo FROM inventario WHERE codigo=?",
        (cod,)
    ).fetchone()
...
def finalizar():
    ...
    cursor.execute(
        "INSERT INTO facturas (subtotal, iva, total, cliente_nombre, cliente_documento, detalles_json) VALUES (?,?,?,?,?,?)",
    ...
    for item in carrito:
        ...
        conn.execute(
            "UPDATE inventario SET stock = stock - ? WHERE codigo = ?",
